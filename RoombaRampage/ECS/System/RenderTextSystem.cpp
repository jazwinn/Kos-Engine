...
namespace ecs {
	...
	void RenderTextSystem::m_Update(const std::string& scene)
	{
		...
		for (int n{}; n < m_vecTextComponentPtr.size(); n++) {
			...
			graphicsPipe->m_textData.push_back(graphicpipe::TextData{ text->m_text,  transform->m_transformation.m_e20 ,transform->m_transformation.m_e21, -rotate, text->m_fontSize, text->m_layer , {red, green, blue}, text->m_fileName});
