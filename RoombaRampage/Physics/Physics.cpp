...
namespace physicspipe {
	...
	void Physics::m_Checker(const Rectangle& obj1, const Rectangle& obj2) {
		...
		std::vector<vector2::Vec2> normalA;
		std::vector<vector2::Vec2> normalB;
		std::vector<vector2::Vec2> axes;
		//for (const auto& edge : edgesA) {
		//	normalA.push_back({ -edge.m_y, edge.m_x }); // Perpendicular vector
		//}

		//for (const auto& edge : edgesB) {
		//	normalB.push_back({ -edge.m_y, edge.m_x });
		//}

		float angleA = obj1.m_rotAngle;
		std::cout << angleA << std::endl;
		float angleB = obj2.m_rotAngle;
		...
		if (((0 <= angleA && angleA < 45.f) || (315.f <= angleA && angleA < 359.f)) || ((-45.f <= angleA && angleA <= 0.f) || (-359.f <= angleA && angleA < -315.f))) {
			//first edge facing top
		...
		else if ((45.f <= angleA && angleA < 135.f) || (-135.f <= angleA && angleA < -45.f)) {
			//first edge facing right
			if (std::abs(smallestAxis.m_x) > std::abs(smallestAxis.m_y)) {
				// Horizontal collision
				if (smallestAxis.m_x > 0) {
		...
		else if ((135.f <= angleA && angleA < 225.f) || (-225.f <= angleA && angleA < -135.f)) {
			//first edge facing bottom
			if (std::abs(smallestAxis.m_x) > std::abs(smallestAxis.m_y)) {
				// Horizontal collision
				if (smallestAxis.m_x > 0) {
		...
		else if ((225.f <= angleA && angleA < 315.f) || (-315.f <= angleA && angleA < -225.f)) {
			//first edge facing left
			if (std::abs(smallestAxis.m_x) > std::abs(smallestAxis.m_y)) {
				// Horizontal collision
				if (smallestAxis.m_x > 0) {
					const_cast<Rectangle&>(obj1).AddCollisionFlag(UP);
			...
			LOGGING_ERROR("Angle not within the range {}", angleA);
		}
