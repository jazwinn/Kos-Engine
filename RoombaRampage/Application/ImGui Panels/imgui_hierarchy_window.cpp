...
unsigned int gui::ImGuiHandler::m_DrawHierachyWindow()
{
    ...
    for (size_t i = 0; i < m_objTextEntries.size(); i++) {
        ...
        if (ImGui::TreeNodeEx(m_objTextEntries[i].c_str() , flag)) {
        ...
        //    ImGui::SetDragDropPayload("Entity", &m_objTextEntries[i], sizeof(ecs::EntityID));
        //    ImGui::Text(m_objTextEntries[i].c_str());
